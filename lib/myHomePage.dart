...
class _MyHomePageState extends State<MyHomePage> {
                ...
                children: [
                  const SizedBox(
                   ...
                   Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      userCard(userName: "User 1", path: "x",color: red,),
                      SizedBox(
                        width: 20,
                      ),
                      userCard(userName: "User 2", path: "o",color: orange,)
                    ],
                  ),
                  const SizedBox(
                    height: 20,
  ...
  bool checkGame() {
    ...
    if (moveList[0] == moveList[1] && moveList[0] == moveList[2]) {
      if (moveList[0] != Colors.white) {setState(() {
        winner = moveList[0];
      }); return true;};
    };
    ...
    if (moveList[1] == moveList[4] && moveList[1] == moveList[7]) {
      if (moveList[1] != Colors.white) {setState(() {
        winner = moveList[1];
      }); return true;};
    };
    if (moveList[2] == moveList[5] && moveList[2] == moveList[8]) {
      if (moveList[2] != Colors.white) {setState(() {
        winner = moveList[2];
      }); return true;};
    };
    if (moveList[3] == moveList[4] && moveList[3] == moveList[5]) {
      if (moveList[3] != Colors.white) {setState(() {
        winner = moveList[3];
      }); return true;};
    };
    if (moveList[6] == moveList[7] && moveList[6] == moveList[8]) {
      if (moveList[6] != Colors.white) {setState(() {
        winner = moveList[6];
      }); return true;};
    };
